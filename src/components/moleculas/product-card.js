...
export default function Card({ data: { stockStatus, slug, databaseId, name, image, price, regularPrice, salePrice, acf, uri }, className }) {
  ...
  return (
    <Wrapper className={`card anim ${className}`}>
      <Link aria-label={name} href={uri} className="shop-link" />
      ...
      <div className={stockStatus === 'IN_STOCK' ? "buttons" : 'buttons disabled'}>
        ...
        <button aria-label={`Dodaj do koszyka ${name}`} disabled={stockStatus !== 'IN_STOCK'} onClick={clickHandler} className="cart">
          <span>
