...
export default function Hero({ data: { stockStatus, stockQuantity = '1', image, galleryImages, name, id, price, regularPrice, salePrice, databaseId, description = ' ', attributes, slug } }) {
  ...
  function enforceMinMax({ currentTarget: el }) {
    if (el.value == '0') {
      ...
      if (parseInt(el.value) < parseInt(el.min)) {
        setQuantity(Number(el.min))
      } else if (parseInt(el.value) > parseInt(el.max)) {
        setQuantity(Number(el.max))
      } else {
        setQuantity(Number(el.value))
      }
    } else {
      setQuantity(Number(el.value))
    }
      ...
      <div className="container">
        ...
        <div className="content">
          ...
          {stockStatus === 'IN_STOCK' && (
            <p className="count">Ilość na stanie: {stockQuantity}</p>
          )}
          {stockStatus === 'IN_STOCK' ? (
            <div className="counter">
              <button aria-label='Dodaj więcej przedmiotów do koszyka' disabled={quantity >= stockQuantity} onClick={() => { setQuantity(quantity >= stockQuantity ? stockQuantity : quantity + 1) }} className="up">
              ...
              <button aria-label='Dodaj mniej przedmiotów do koszyka' disabled={quantity <= 1} onClick={() => { setQuantity(quantity <= 1 ? 1 : quantity - 1) }} className="down">
                <svg width="30" height="16" viewBox="0 0 30 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <label>
                <span className="dn">Ilość predmiotów</span>
              ...
              <ButtonFilled disabled={quantity < 1} onClick={clickHandler} as='button'>
                <span>
                  Dodaj do koszyka
                </span>
              </ButtonFilled>
